...
TMUX_PROGRAM="/home/ayush.gangwani/.local/bin/tmux"
TMUX_CONF="/home/ayush.gangwani/.config/tmux/tmux.conf"
TMUX_CONF_LOCAL="/home/ayush.gangwani/.config/tmux/tmux.theme"
TMUX_THEME_SCRIPT="/home/ayush.gangwani/.config/tmux/apply_theme.sh"
  ...
  case "$_uname_s" in
    ...
    *Linux*)
      ps -t "$pane_tty" --sort=lstart -o user=XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX -o pid= -o ppid= -o command= | awk -v pane_pid="$pane_pid" '
        ((/ssh/ && !/-W/ && !/tsh proxy ssh/ && !/sss_ssh_knownhostsproxy/ && !/gssh-agent/) || !/ssh/) && !/tee/ {
...
_apply_theme() {
  ...
  set_titles_string=$(printf '%s' "${tmux_conf_theme_terminal_title:-$(tmux show -gv set-titles-string)}" | sed \
    ...
    -e "s%#{username_ssh}%#(cat '$TMUX_THEME_SCRIPT' | sh -s _username '#{pane_pid}' '#{b:pane_tty}' true '#D')%g" \
    -e "s%#{hostname_ssh}%#(cat '$TMUX_THEME_SCRIPT' | sh -s _hostname '#{pane_pid}' '#{b:pane_tty}' true false '#h' '#D')%g" \
    -e "s%#{hostname_full_ssh}%#(cat '$TMUX_THEME_SCRIPT' | sh -s _hostname '#{pane_pid}' '#{b:pane_tty}' true true '#H' '#D')%g")
  ...
  status_left=$(printf '%s' "${status_left-$(tmux show -gv status-left)}" | sed \
    ...
    -e "s%#{circled_session_name}%#(cat '$TMUX_THEME_SCRIPT' | sh -s _circled '#S')%g" \
    -e "s%#{root}%#{?#{==:#(cat '$TMUX_THEME_SCRIPT' | sh -s _username '#{pane_pid}' '#{b:pane_tty}' '#D'),root},$tmux_conf_theme_root,}%g")
  ...
  _pkillf "cat '$TMUX_THEME_SCRIPT' \| sh -s _battery"
  _battery_info
  if [ "$battery_charge" != 0 ]; then
    case "$status_left $status_right" in
      *'#{battery_'*|*'#{?battery_'*)
        status_left=$(echo "$status_left" | sed -E \
          ...
          -e "s%#\{battery_bar\}%#(nice cat '$TMUX_THEME_SCRIPT' | sh -s _bar '$(printf '%s' "$tmux_conf_battery_bar_palette" | tr ',' ';')' '$tmux_conf_battery_bar_symbol_empty' '$tmux_conf_battery_bar_symbol_full' '$tmux_conf_battery_bar_length' '#{@battery_charge}' '#{client_width}')%g" \
          -e "s%#\{battery_hbar\}%#(nice cat '$TMUX_THEME_SCRIPT' | sh -s _hbar '$(printf '%s' "$tmux_conf_battery_hbar_palette" | tr ',' ';')' '#{@battery_charge}')%g" \
          -e "s%#\{battery_vbar\}%#(nice cat '$TMUX_THEME_SCRIPT' | sh -s _vbar '$(printf '%s' "$tmux_conf_battery_vbar_palette" | tr ',' ';')' '#{@battery_charge}')%g" \
        ...
        status_right=$(echo "$status_right" | sed -E \
          ...
          -e 's%#\{(\?)?battery_status%#\{\1@battery_status%g' \
          -e 's%#\{(\?)?battery_percentage%#\{\1@battery_percentage%g')
        status_right="#(echo; nice cat '$TMUX_THEME_SCRIPT' | sh -s _battery_status '$tmux_conf_battery_status_charging' '$tmux_conf_battery_status_discharging')$status_right"
        interval=60
        if [ "$_tmux_version" -ge 320 ]; then
          tmux run -b "trap '[ -n \"\$sleep_pid\" ] && kill -9 \"\$sleep_pid\"; exit 0' TERM; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _battery_info; sleep $interval & sleep_pid=\$!; wait \"\$sleep_pid\"; sleep_pid=; done"
        elif [ "$_tmux_version" -ge 280 ]; then
          status_right="#(echo; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _battery_info; sleep $interval; done)$status_right"
        elif [ "$_tmux_version" -gt 240 ]; then
          status_right="#(echo; while :; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _battery_info; sleep $interval; done)$status_right"
        else
          status_right="#(nice cat '$TMUX_THEME_SCRIPT' | sh -s _battery_info)$status_right"
        fi
        ;;
    esac
  fi
  ...
  _pkillf "cat '$TMUX_THEME_SCRIPT' \| sh -s _uptime"
  case "$status_left $status_right" in
    *'#{uptime_'*|*'#{?uptime_'*)
      ...
      if [ "$_tmux_version" -ge 320 ]; then
        tmux run -b "trap '[ -n \"\$sleep_pid\" ] && kill -9 \"\$sleep_pid\"; exit 0' TERM; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _uptime; sleep $interval & sleep_pid=\$!; wait \"\$sleep_pid\"; sleep_pid=; done"
      elif [ "$_tmux_version" -ge 280 ]; then
        status_right="#(echo; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _uptime; sleep $interval; done)$status_right"
      elif [ "$_tmux_version" -gt 240 ]; then
        status_right="#(echo; while :; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _uptime; sleep $interval; done)$status_right"
      else
        status_right="#(nice cat '$TMUX_THEME_SCRIPT' | sh -s _uptime)$status_right"
      fi
      ;;
  esac

  _pkillf "cat '$TMUX_THEME_SCRIPT' \| sh -s _loadavg"
  case "$status_left $status_right" in
    *'#{loadavg'*|*'#{?loadavg'*)
      status_left=$(echo "$status_left" | sed -E \
      ...
      interval=$(tmux show -gv status-interval)
      if [ "$_tmux_version" -ge 320 ]; then
        tmux run -b "trap '[ -n \"\$sleep_pid\" ] && kill -9 \"\$sleep_pid\"; exit 0' TERM; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _loadavg; sleep $interval & sleep_pid=\$!; wait \"\$sleep_pid\"; sleep_pid=; done"
      elif [ "$_tmux_version" -ge 280 ]; then
        status_right="#(echo; while [ x\"\$('$TMUX_PROGRAM' -S '#{socket_path}' display -p '#{l:#{pid}}')\" = x\"#{pid}\" ]; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _loadavg; sleep $interval; done)$status_right"
      elif [ "$_tmux_version" -gt 240 ]; then
        status_right="#(echo; while :; do nice cat '$TMUX_THEME_SCRIPT' | sh -s _loadavg; sleep $interval; done)$status_right"
      else
        status_right="#(nice cat '$TMUX_THEME_SCRIPT' | sh -s _loadavg)$status_right"
      fi
      ;;
  esac
